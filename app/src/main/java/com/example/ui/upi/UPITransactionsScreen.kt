...
fun UPITransactionsScreen(
    onAddTransaction: (String, String) -> Unit = { _, _ -> },
    onRegisterUpi: (String) -> Unit = { _ -> }
) {
                    ...
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Column(modifier = Modifier.padding(14.dp)) {
                            ...
                            Row(modifier = Modifier.fillMaxWidth().padding(top = 12.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                Button(onClick = { onAddTransaction(transaction.amount, transaction.message) }, modifier = Modifier.weight(1f)) {
                                    Text("Add Transaction")
                                }
                                Button(onClick = { onRegisterUpi(transaction.sender) }, modifier = Modifier.weight(1f)) {
                                    Text("Register UPI")
                                }
                            }
                        }
