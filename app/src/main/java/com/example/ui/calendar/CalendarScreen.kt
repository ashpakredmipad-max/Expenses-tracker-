...
fun CalendarScreen(
    ...
    val expensesByDay = transactions
        .filter { it.type.equals("EXPENSE", true) && isSameMonth(it.date, calendar) }
        ...
        LazyVerticalGrid(
            ...
            items(cells) { day ->
                if (day == null) {
                    ...
                    CalendarDay(
                        day = day,
                        expenseInPaise = expensesByDay[day] ?: 0L
                    )
                }
            }
        }
...
private fun CalendarDay(day: Int, expenseInPaise: Long) {
    Card(
        ...
        Column(
            ...
            Text(
                text = if (expenseInPaise > 0) "₹${formatAmount(expenseInPaise)}" else "—",
                style = MaterialTheme.typography.labelSmall,
                color = if (expenseInPaise > 0) MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.outline,
...
private fun isSameMonth(timestamp: Long, selected: Calendar): Boolean {
    ...
    return c.get(Calendar.YEAR) == selected.get(Calendar.YEAR) &&
...
private fun formatAmount(paise: Long): String {
    ...
    val decimals = (paise % 100).toString().padStart(2, '0')
    return "$rupees.$decimals"
}
