...
fun WalletManagerScreen(onNavigateBack: () -> Unit) {
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = { editingWallet = null; name = ""; showEditor = true }, shape = RoundedCornerShape(16.dp)) {
            ...
            items(wallets, key = { it.id }) { wallet ->
                Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(16.dp), elevation = CardDefaults.cardElevation(1.dp)) {
                    Row(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                        Icon(Icons.Default.AccountBalanceWallet, null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.size(28.dp))
                        Spacer(Modifier.size(14.dp))
                        Column(Modifier.weight(1f).clickable { editingWallet = wallet; name = wallet.name; showEditor = true }) {
                        ...
                        IconButton(onClick = { deleteWallet = wallet }) { Icon(Icons.Default.Delete, "Delete", tint = MaterialTheme.colorScheme.error) }
                    }
                }
            }
        }
