    ...
    Scaffold(
        ...
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Move money between wallets", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
            }

            Text("From Wallet", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
            WalletSelector(wallets, fromWallet) { fromWallet = it; if (toWallet?.id == it?.id) toWallet = wallets.firstOrNull { w -> w.id != it?.id } }

            Text("To Wallet", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
            ...
            OutlinedTextField(
                value = amount,
                onValueChange = { if (it.isEmpty() || it.matches(Regex("^\\d*\\.?\\d{0,2}$"))) { amount = it; error = null } },
            ...
            Button(
                ...
                Spacer(Modifier.width(8.dp))
                Text("Transfer Amount", fontWeight = FontWeight.Bold)
            }
        }
    }
}
...
private fun WalletSelector(wallets: List<Wallet>, selected: Wallet?, onSelect: (Wallet) -> Unit) {
    if (wallets.isEmpty()) {
        Text("No other wallet available")
    } else {
                ...
                Surface(
                    ...
                    Row(Modifier.padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Column {
                            ...
                            Text("Balance: ₹${String.format("%.2f", wallet.balance)}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                    }
                }
            }
